...
from layers import EncoderBlock, DecoderBlock, SelfAttention, Conv1x1
...
class Vae(flow.Model):
    ...
    def init_configs(self):
        ...
        self.attention_dropout_rate = float(self.config.get("model.attention_dropout_rate", "0.1"))
        self.n_heads = int(self.config.get("model.n_heads", "4"))
        self.n_blocks = int(self.config.get("model.n_blocks", "3"))
        self.alpha = int(self.config.get("model.alpha", "alpha"))
    ...
    def encode_block(self, units):
        block = [
            ...
            Conv1x1(filters=units),
        ]
        return block

    def decode_block(self, units):
        block = [
            SelfAttention(
                flange_size=(1, 1),
                qk_units=units,
    ...
    def encoder(self):
        ...
        def positional_emb_add(i):
            with tf.variable_scope("encode_scope", reuse=tf.AUTO_REUSE):

                return lambda x: x + tf.get_variable(
                    name="positional_emb_{}".format(i),
        ...
        with tf.variable_scope("encode_scope", reuse=tf.AUTO_REUSE):
            self._encode_blocks.append(
                tf.keras.layers.Lambda(lambda x: tf.space_to_depth(x, 2))
            )
            for i in range(self.n_blocks):
                self._encode_blocks.append(
                    tf.keras.layers.Conv2D(
                        filters=(self.alpha * (2**i)), 
                        kernel_size=(3, 3), 
                        strides=(1, 1), 
                ...
                self._encode_blocks.append(
                    tf.keras.layers.Lambda(
                        positional_emb_add(i),
                    )
                )

                self._encode_blocks.extend(
                    self.encode_block((self.alpha *(2**i)))
                )
                ...
                if i < self.n_blocks - 1:
                    self._encode_blocks.append(tf.nn.relu)
                else:
                    self._encode_blocks.append(
                        tf.keras.layers.Lambda(
                            lambda x: tf.nn.tanh(x) * tf.get_variable(
                                name="encode_scale", 
                                shape=[1] + x.shape.as_list()[1:],
    ...
    def decoder(self):
        self._decode_blocks = list()
        def positional_emb_add(i):
            with tf.variable_scope("decode_scope", reuse=tf.AUTO_REUSE):
                # print("*****>>>", i)
                return lambda x: x + tf.get_variable(
                    name="dpositional_emb_{}".format(i),
                    shape=[1] + x.shape.as_list()[1:],
                    dtype=tf.float32,
                    initializer=tf.initializers.truncated_normal(stddev=0.02),
                    trainable=True
        ...
        with tf.variable_scope("decode_scope", reuse=tf.AUTO_REUSE):
            for i in range(self.n_blocks - 1, -1, -1):
                ...
                if i > 0:
                    self._decode_blocks.extend(
                        self.decode_block((self.alpha * (2**i)))
                    )
                    self._decode_blocks.append(tf.nn.relu)
                else:
                    self._decode_blocks.extend(
                        # self.decode_block(4)
                        self.decode_block((self.alpha * (2**i)))
                    )
                    # self._decode_blocks.append(tf.nn.sigmoid)
                    self._decode_blocks.append(tf.nn.relu)
            ...
            self._decode_blocks.append(tf.nn.sigmoid)
            self._decode_blocks.append(
                tf.keras.layers.Lambda(lambda x: tf.depth_to_space(x, 2))
            )
    ...
    def get_loss_and_metrics(self, _inputs):
        ...
        kl_loss = -0.5 * tf.reduce_sum(1 + std - tf.square(mean) - tf.exp(std), axis=[1, 2, 3])
