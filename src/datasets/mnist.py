...
class MnistDS(DS):
    ...
    def __next__(self):
        ...
        resolution = [ int(aux) for aux in self.config.get("dataset.patch_size", "28,28").strip().split(",")]
        x = downsample(x, resolution)
        x = x.astype(np.float32)
        self._idx += 1
        return (np.mean(x/255, axis=-1, keepdims=True),)


def downsample(x, resolution):
    # print(">>>>>>", x.shape, resolution)
    # assert x.dtype == np.float32
    # assert x.shape[0] % resolution[0] == 0
    # assert x.shape[1] % resolution[1] == 0
    # if x.shape[1] == x.shape[2] == resolution[0]:
    #     return x
    # s = x.shape
    # x = np.reshape(x, [resolution[0], s[0] // resolution[1],
    #                    resolution[0], s[1] // resolution[1], s[2]])
    # x = np.mean(x, (1, 3))
    from skimage.transform import resize
    x = resize(x, resolution)
    return x
