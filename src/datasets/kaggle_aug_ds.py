...
class KaggleAugDS(DS):
    """Seismogram dataset iterator.
    Reads the dataset and iterates over it.
    """

    def __init__(self, inputs_config, config=None, path=None, partition="train", *args, **kwargs):
        ...
        self.dataset = None
        # self.build_dataset()
        super().__init__(inputs_config, config, path, None, *args, **kwargs)
    ...
    def __getitem__(self, item):
        if isinstance(item, int):
            return self._y[item], self._meta[item]
